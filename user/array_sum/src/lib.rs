...
pub fn main() -> Result<()> {
    ...
    while count < 13 {
        // println!("hhh{}", count);
        let mut buffer2: DataBuffer<Reader2Arraysum> = DataBuffer::with_slot(format!("part-{}", count+1));
        let reader: DataBuffer<Reader2Arraysum> = DataBuffer::from_buffer_slot(format!("part-{}", count)).expect("missing input data.");
        buffer2.raw_data = reader.raw_data.clone();
        count += 1;
    }
    // println!("hhh100");
    let reader2: DataBuffer<Reader2Arraysum> = DataBuffer::from_buffer_slot(format!("part-{}", 13)).expect("missing input data.");
    let result = reader2.raw_data.clone();
    // println!("hhh101");
    Ok(().into())
}
